...
import { getVisitsForHouse, getFrequentVisitorsForHouse, markVisitorFrequent } from '../supabase/db'
import { supabase } from '../supabase/config'
import Navbar from '../components/Navbar'
import VisitorCard from '../components/VisitorCard'
import LoadingSpinner from '../components/LoadingSpinner'
import { toast } from 'react-toastify'
import { Users, UserCheck, History, Star, AlertCircle } from 'lucide-react'
...
export default function OwnerDashboard() {
  ...
  // Build visitor+visit pairs for display
  ...
  const insideCards = buildCards(activeVisits)
  const historyCards = buildCards(historyVisits)
            ...
            {activeTab === 'inside' && (
              <div className="space-y-3">
                {insideCards.length === 0 ? (
                  ...
                  insideCards.map(({ visitor, visit }) => (
                    <VisitorCard
                      key={visit.id}
                      visitor={visitor}
                      visit={visit}
                      showActions
                      onMarkFrequent={handleMarkFrequent}
                    />
                  ))
                )}
              </div>
            ...
            {activeTab === 'history' && (
              <div className="space-y-3">
                {historyCards.length === 0 ? (
                  ...
                  historyCards.map(({ visitor, visit }) => (
                    <VisitorCard
                      key={visit.id}
                      visitor={visitor}
                      visit={visit}
              ...
              <div className="space-y-3">
                {frequentVisitors.length === 0 ? (
                  ...
                  frequentVisitors.map(visitor => (
                    <VisitorCard
                      key={visitor.id}
                      visitor={{ ...visitor, houses: { house_number: '' } }}
                    />
                  ))
                )}
              </div>
